...
const Events = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-['Plus Jakarta Sans'] scroll-smooth">
            <PageHero
                title="Campus"
                italicTitle="Vibes"
                tag="Living The Legacy"
                subtitle="A vibrant world of sports, culture, and innovation shaping well-rounded personalities every day."
            ...
            <div className="max-w-7xl mx-auto px-4 mb-32">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {stats.map((stat, idx) => (
                        ...
                        >
                            <h2 className="text-6xl font-extrabold text-indigo-600 mb-2">{stat.val}</h2>
                            <p className="text-xl font-extrabold text-slate-900 mb-4 uppercase tracking-widest">{stat.label}</p>
                            <p className="text-slate-500 text-sm font-extrabold">{stat.desc}</p>
                        </motion.div>
                    ))}
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 mb-16">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-12">
                    <h2 className="text-4xl font-extrabold text-slate-900">Life at <span className="text-indigo-600 underline decoration-amber-400">Gyan Sagar</span></h2>
                    <div className="flex flex-wrap justify-center gap-3">
                        {['All', 'Cultural', 'Academic', 'Sports', 'Creative'].map(cat => (
                            <button
                                key={cat}
                                onClick={() => setSelectedCategory(cat)}
                                className={`px-6 py-2.5 rounded-full font-extrabold text-xs transition-all tracking-widest uppercase ${selectedCategory === cat ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 border border-slate-200'}`}
                            >
                                {cat}
                            </button>
                        ))}
                            ...
                            >
                                <div className="aspect-video overflow-hidden relative">
                                    ...
                                    <div className="absolute bottom-8 left-8 z-20 text-white">
                                        <span className="bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest border border-white/30">
                                            {activity.date}
                                        </span>
                                    </div>
                                </div>
                                <div className="p-12">
                                    <div className="flex justify-between items-center mb-6">
                                        <h3 className="text-3xl font-extrabold text-slate-900 leading-tight">{activity.title}</h3>
                                        <Camera className="w-6 h-6 text-slate-200 hover:text-indigo-600 cursor-pointer transition-colors" />
                                    </div>
                                    <p className="text-slate-500 text-lg font-medium leading-relaxed mb-8">
                                        {activity.description}
                                    </p>
                                    <button className="flex items-center gap-3 font-extrabold text-indigo-600 hover:gap-5 transition-all">
            ...
            <div className="bg-slate-900 py-20 md:py-32 mt-20 md:mt-32 relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 flex flex-col lg:flex-row items-center gap-12 md:gap-20">
                    <div className="lg:w-1/2">
                        <h2 className="text-5xl md:text-7xl font-extrabold text-white mb-8 tracking-tighter">Beyond the <br /><span className="text-amber-400">Classroom</span></h2>
                        <div className="space-y-6 md:space-y-8">
                            {[
                                ...
                                <div key={idx} className="flex gap-4 md:gap-6 group">
                                    <div className="shrink-0 w-10 h-10 md:w-12 md:h-12 bg-white/10 rounded-xl md:rounded-2xl flex items-center justify-center text-amber-400 border border-white/10 group-hover:bg-amber-400 group-hover:text-slate-900 transition-all">
                                        {React.cloneElement(item.icon, { className: "w-4 h-4 md:w-5 md:h-5" })}
                                    </div>
                                    <div>
                                        <h4 className="text-white text-lg md:text-xl font-extrabold mb-1 md:mb-2">{item.title}</h4>
                    ...
                    <div className="lg:w-1/2 relative w-full px-4 md:px-0">
                        <div className="absolute inset-0 bg-indigo-500/20 rounded-[2rem] md:rounded-[4rem] blur-[50px] md:blur-[100px]"></div>
                        <div className="relative bg-white/5 backdrop-blur-md p-3 md:p-6 rounded-[2rem] md:rounded-[5rem] border border-white/10 shadow-2xl transform md:rotate-3 hover:rotate-0 transition-all duration-700">
                            <img
                                src="https://images.unsplash.com/photo-1541829070764-84a7d30dee6d?auto=format&fit=crop&q=80&w=800"
                                alt="Campus Aura"
                                className="w-full h-[350px] md:h-[600px] object-cover rounded-[1.5rem] md:rounded-[4rem]"
                            />
                            <div className="absolute -bottom-10 -left-10 bg-indigo-600 p-8 rounded-[2.5rem] shadow-2xl hidden md:block">
            ...
            <div className="max-w-4xl mx-auto px-4 py-20 md:py-32 text-center">
                <h3 className="text-2xl md:text-3xl font-black text-slate-800 mb-6 md:mb-8">Want to Experience Our Campus?</h3>
                <p className="text-slate-500 font-medium text-base md:text-lg mb-8 md:mb-12">
                ...
                <div className="flex flex-col sm:flex-row justify-center gap-4 md:gap-6">
                    <button className="px-8 md:px-12 py-4 md:py-5 bg-indigo-600 text-white font-black rounded-2xl md:rounded-[2rem] shadow-2xl shadow-indigo-200 hover:bg-slate-900 transition-all flex items-center justify-center gap-3 text-sm md:text-base">
                        Book a School Tour <ArrowRight className="w-5 h-5" />
                    </button>
                    <button className="px-8 md:px-12 py-4 md:py-5 bg-white border-2 border-slate-100 text-slate-600 font-black rounded-2xl md:rounded-[2rem] hover:bg-slate-50 transition-all flex items-center justify-center gap-3 text-sm md:text-base">
